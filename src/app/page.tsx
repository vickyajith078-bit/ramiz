...
export default function Home() {
  ...
  return <main>
    ...
    {enquiry && <div className="modal-backdrop" onClick={() => setEnquiry(false)}><div className="enquiry-modal" onClick={(e) => e.stopPropagation()}><button className="modal-close" onClick={() => setEnquiry(false)} aria-label="Close enquiry"><X /></button>{sent ? <div className="success-message"><Sparkles size={30} /><h2>Thank you.</h2><p>Your enquiry has been noted. We&apos;ll be in touch soon.</p><button className="button button-gold" onClick={() => setEnquiry(false)}>Close</button></div> : <><p className="eyebrow eyebrow-dark">Let&apos;s connect</p><h2>Make an enquiry</h2><form onSubmit={(e) => { e.preventDefault(); const formData = new FormData(e.currentTarget); const subject = encodeURIComponent(String(formData.get("subject"))); const body = encodeURIComponent(`Name: ${formData.get("name")}\nPhone: ${formData.get("phone")}\nEmail: ${formData.get("email")}\n\n${formData.get("message")}`); window.location.href = `mailto:ajithssoftwareengineer@gmail.com?subject=${subject}&body=${body}`; }}><label>Name<input required name="name" /></label><label>Phone number<input required name="phone" type="tel" /></label><label>Email<input required name="email" type="email" /></label><label>Subject<input required name="subject" /></label><label>Message<textarea required name="message" rows={4} /></label><button className="button button-dark" type="submit">Submit enquiry <ArrowRight size={16} /></button></form></>}</div></div>}
  </main>;
}
